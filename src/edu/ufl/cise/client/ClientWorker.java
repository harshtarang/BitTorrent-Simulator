...
public class ClientWorker extends ReadWorker implements Runnable {
	...
	public void run() {
		...
		try {
			...
			String logMessage = "Peer " + this.currPeerId
					+ " makes a connection to Peer " + this.peerID;
			...
			while (LISTENING) {
				...
				if (isHandShakeMessage(firstFour)) { // Check the type of
					...
					// System.out.println("Read bytes should be 14 : " +
					// bytesRead);
					...
					if (headerString.equalsIgnoreCase(HandshakeMessage.HEADER)) {
						...
						// bytesRead = in.read(temp, 18, 14); // read the
						// remaining bytes
						for (int i = 18; i < 32; i++) {
							temp[i] = (byte) in.read();
						}
					...
					// System.out.println("Read bytes should be " + len + " : "
					// + bytesRead);
